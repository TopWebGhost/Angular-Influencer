...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            old_user_obj = orm.UserIdMap.objects.get(user_id=25)
            new_user_obj = orm['auth.User'].objects.get(username='atulsingh')
        except:
            pass
        else:
            for item in orm.UserOperations.objects.all():
                if item.user_id == old_user_obj:
                    item.userid_new = new_user_obj
                    item.save()
                else:
                    item.delete()
            for item in orm.WishlistItem.objects.all():
                if item.user_id == old_user_obj:
                    item.userid_new = new_user_obj
                    item.save()
                else:
                    item.delete()
            for item in orm.CombinationOfUserOps.objects.all():
                if item.user_id == old_user_obj:
                    item.userid_new = new_user_obj
                    item.save()
                else:
                    item.delete()
            for item in orm.PricingTasks.objects.all():
                if item.user_id == old_user_obj:
                    item.userid_new = new_user_obj
                    item.save()
                else:
    ...
    models = {
        ...
        'debra.ssitemstats': {
            ...
            'tdate': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2012, 2, 24, 15, 1, 20, 711254)'}),
