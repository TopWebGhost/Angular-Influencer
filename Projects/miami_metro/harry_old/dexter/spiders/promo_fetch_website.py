...
class PromoFetchWebsiteSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        if 'victoriassecret' in url:
            self._extract_promo_text_from_homepage(response, "Victoria's Secret")
            pro = PromoRawText.objects.filter(store__name = "Victoria's Secret", insert_date = self.today)
            for p in pro:
                popmail._common_work("Victoria's Secret", self.today, self.time, 'victoriassecret',['code ', 'codes '], smart_str(p.raw_text).lower())
        ...
        if 'urbanoutiffers' in url:
            store_name = 'urbanoutiffers.com/'

        if 'nastygal' in url:
            store_name = 'nastygal.com/'

        if 'neimanmarcus' in url:
            store_name = 'neimanmarcus.com/'

        if 'nordstrom' in url:
            store_name = 'shop.nordstrom.com/'

        if 'bloomingdales' in url:
            store_name = 'bloomingdales.com/'

        if 'lanebryant' in url:
            store_name = 'lanebryant.com/'
