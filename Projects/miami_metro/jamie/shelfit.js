javascript: var d=document, 
	...
	{ 
		if (l_store_chk1 != -1)
		{
			...
			else
			{
				if (size_vals.length == 1)
				{
					...
					size=size_vals[0].options[size_idx].value.replace(/(^\s*)|(\s*$)/g, '');
				}
				else if (size_vals.length == 2)
				{
					size_idx=size_vals[1].selectedIndex;
					size=size_vals[1].options[size_idx].value.replace(/(^\s*)|(\s*$)/g, '');	
				}
				color_idx=1; 
				if ((size_idx > 0) || ((size_idx == 0) && (size))) 
				{ 
					...
					u=f+'?u='+e(l.href)+'&s='+e(size)+'&c='+e(color)+'&q='+e(qtity)+'&imgurl='+e(img_url);
				}
			}
		}	 
		...
		{
			if (((size_idx > 0) || ((size_idx == 0) && (size))) && (color_idx > 0))
			{
				a=function(){
